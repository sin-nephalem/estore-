<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@page import="com.briup.app.estore.bean.Orderline,java.util.Map"%>
...
<!--文件体开始-->
        ...
        <c:forEach items="${map}" var="p">
        	 <tr>
				<form method="post" action="EditOrderlineServlet" name="f1">
				<input type="hidden" name="productid" value="${p.key}">
				<input type="hidden" name="number" value="${p.value.num }">
				<td class=tablebody2 valign=middle align=center width="">${p.key}</td>
				<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a href="productDetail.jsp?productid=2">${p.value.book.name }</a></td>
				<td class=tablebody2 valign=middle align=center width="">${p.value.book.price}</td>
				<td class=tablebody1 valign=middle align=center width=""><input type="text" name="num" value="${p.value.num }" size="4" onblur="javascript:if(this.value<1){alert('对不起，产品数量不能小于 1 ');this.focus();}else{number.value=this.value;}"/></td>
				<td class=tablebody2 valign=middle align=left width="">&nbsp;&nbsp;${p.value.num*p.value.book.price}</td>
				<td class=tablebody1 valign=middle align=center width="">
				<input type="button" value="取消" onclick="javascript:window.location='DelOrderlineServlet?lineid=${p.key}';"> <input type="submit" value="保存修改"></td>
			</form>
		</tr>
        </c:forEach>
		...
		<tr>
			...
			<td class=tablebody1 valign=middle align=left width="">&nbsp;&nbsp;<font color="#ff0000"><b><%=sc.getCost() %></b></font></td>
		...
		<tr>
			...
			<input type="button" value="清空购物车" onclick="javascript:window.location='ClearCartServlet';"></td>
		</tr>
                </table><br>
<!--文件尾开始-->
